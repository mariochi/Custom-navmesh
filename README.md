...
2. Crie um GameObject vazio na cena e adicione o componente `NavMeshJobManager`
   ([Runtime/NavMeshJobManager.cs](Runtime/NavMeshJobManager.cs)). Só pode existir um por cena.
...
4. Em cada personagem que deve navegar, adicione `CustomNavMeshAgent`
   ([Runtime/CustomNavMeshAgent.cs](Runtime/CustomNavMeshAgent.cs)) — **não** use o
...
- **Sem `asmdef` próprio.** O código está em `Assets/Scripts/CustomNavMesh` dentro do
  assembly padrão (`Assembly-CSharp`). Pra projetos maiores, vale mover pra um
  Assembly Definition próprio (melhora tempo de recompilação incremental).
...
| Arquivo | Papel |
|---|---|
| `NavMeshJobConstants.cs` | Constantes compartilhadas (`MaxCorridorPoints`, `MaxNavMeshAreas`). |
| `Data/NavMeshGraph.cs` | Struct com o NavMesh em NativeArrays. |
| `Data/NavMeshGraphBuilder.cs` | Ponte main-thread: `NavMesh.CalculateTriangulation()` → `NavMeshGraph`, incluindo solda de vértices duplicados entre tiles. |
| `Data/NavMeshSpatialGrid.cs` | Grid uniforme XZ pra localizar triângulo por posição. |
| `Jobs/BuildTriangleAdjacencyJob.cs` | Descobre vizinhos de cada triângulo (1x por bake). |
| `Jobs/BuildTriangleGridJob.cs` | Monta o `NavMeshSpatialGrid` (1x por bake). |
| `Jobs/PathRequest.cs` | Struct de pedido de caminho. |
| `Jobs/FindPathsBatchJob.cs` | A* + funnel, em lote, por frame. |
| `Jobs/BuildAgentSpatialHashJob.cs` | Grid de agentes por frame (pra avoidance). |
| `Jobs/AvoidanceAndMoveJob.cs` | Avoidance + integração de movimento + clamp na malha; branch corredor individual vs. flow field. |
| `Jobs/ComputeFlowFieldJob.cs` | Dijkstra a partir do destino sobre todos os triângulos, pra `MoveGroupWithFlowField`. |
| `Queries/NavMeshQueryUtil.cs` | `ClosestPointOnTriangle`, `FindNearestTriangle`, `IsAreaAllowed`. |
| `Queries/NativeMinHeap.cs` | Heap binário usado como open list do A*. |
| `Queries/Funnel.cs` | String-pulling (Simple Stupid Funnel Algorithm). |
